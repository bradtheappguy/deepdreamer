...
- (NSString *)pathForResource:(NSString *)path {
	NSString *p  =[NSString stringWithFormat:@"%@/%@%@", [[NSBundle mainBundle] resourcePath], appFolder, path];
  return p;
}
...
- (void)run:(CADisplayLink *)sender {
  ...
  if (count >= 15) {
    ...
    if (!l) {
      l= (UILabel *)[self.window.rootViewController.view viewWithTag:690];
    }
    l.text = [NSString stringWithFormat:@"%f",1/frameDuration];
    count = 0;
  }
}
