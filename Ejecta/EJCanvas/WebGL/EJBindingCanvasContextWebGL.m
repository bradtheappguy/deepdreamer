...
EJ_BIND_FUNCTION(foobar, ctx, argc, argv) {
	if( argc < 5 ) { return NULL; }
	EJ_UNPACK_ARGV(GLsizei width, GLsizei height);
    

//    width/=2;
//    height/=2;
    
	JSTypedArrayType arrayType = JSTypedArrayGetType(ctx, argv[2]);
	if( !EJ_ARRAY_MATCHES_TYPE(arrayType, GL_UNSIGNED_BYTE) ) {
		return NULL;
	}
	...
	size_t size;
	char *pixels = JSTypedArrayGetDataPtr(ctx, argv[2], &size);

  NSString* noisemap = (NSString*)CFBridgingRelease(JSStringCopyCFString(kCFAllocatorDefault, JSValueToStringCopy(ctx, argv[3], NULL)));
  NSString* options = (NSString*)CFBridgingRelease(JSStringCopyCFString(kCFAllocatorDefault, JSValueToStringCopy(ctx, argv[4], NULL)));


	GLuint bytesPerPixel = EJGetBytesPerPixel(GL_UNSIGNED_BYTE, GL_RGBA);
  ...
  for(int y = 0; y < height; y++)
    ...
    for(int x = 0; x <width * 4; x++)
      {
      buffer2[(y) * (int)width * 4 + x] = pixels[y * 4 * (int)width + x];
      }
    }
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:@"WEBGL_IMAGE_CAPTURED" object:nil userInfo:@{@"image":myImage,
                                                                                                           @"noisemap":noisemap,
                                                                                                           @"options":options
                                                                                                           }];
	return NULL;
...
EJ_BIND_FUNCTION(readPixels, ctx, argc, argv) {
	...
	size_t size;
	char *pixels = JSTypedArrayGetDataPtr(ctx, argv[6], &size);
