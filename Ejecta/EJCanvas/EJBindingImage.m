...
- (void)beginLoad {
    ...
    UIImage *image = [(IMAppDelegate *)[[UIApplication sharedApplication] delegate] image];
    texture = [[EJTexture alloc] initWithUIImage:image];
    [[EJNonRetainingProxy proxyWithTarget:self] performSelector:@selector(endLoad)];
    return;
  }
	else {
