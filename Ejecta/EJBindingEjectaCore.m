...
EJ_BIND_FUNCTION(log, ctx, argc, argv ) {
  ...
  NSString *s = JSValueToNSString(ctx, argv[0]);
	NSLog( @"JS: %@",  s);
	return NULL;
}

EJ_BIND_FUNCTION(hideProgressLabel, ctx, argc, argv ) {
...
EJ_BIND_FUNCTION(updateProgressLabel, ctx, argc, argv ) {
	if( argc < 1 ) return NULL;
  NSString *label = JSValueToNSString(ctx, argv[0]);
  dispatch_async(dispatch_get_main_queue(),^{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"UPDATE_PROGESS_LABEL" object:label userInfo:nil];
  });
	return NULL;
}

EJ_BIND_FUNCTION(zoomCanvasToFitScreen, ctx, argc, argv ) {
  //   dispatch_async(dispatch_get_main_queue(),^{
  //    [[NSNotificationCenter defaultCenter] postNotificationName:@"ZOOM_CANVAS" object:nil userInfo:nil];
  //  });
	return NULL;
}

EJ_BIND_FUNCTION(seqeuenceDidFinish, ctx, argc, argv ) {
  dispatch_async(dispatch_get_main_queue(),^{
  [[NSNotificationCenter defaultCenter] postNotificationName:@"SEQUENCE_DID_FINISH" object:nil userInfo:nil];
        });
	return NULL;
}
