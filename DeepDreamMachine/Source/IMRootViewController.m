...
@interface IMRootViewController () <MFMailComposeViewControllerDelegate,
IMSharingViewControllerDelegate,
UIDocumentInteractionControllerDelegate,
SYSincerelyControllerDelegate,
UIPopoverControllerDelegate,
IMBrushPickerViewControllerDelegate,
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(imageSettingsDidChange:) name:IMSettingsImageSizeSettingDidChangeNotification object:nil];
...
- (void) openGalleryWithPostID:(NSURL *)url {
    IMWebViewController *webViewController = [[IMWebViewController alloc] initWithNibName:@"IMWebViewController" bundle:nil];
...
- (void) didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation {
    ...
    if (self.popover.isPopoverVisible) {
        [self.popover presentPopoverFromRect:presentedpopoverButton.frame inView:self.view permittedArrowDirections:UIPopoverArrowDirectionDown animated:YES];
    }
}
...
-(IBAction)cameraButtonPressed:(UIButton *)sender {
    ...
    IMImagePickingViewController *vc = [[IMImagePickingViewController alloc] initWithDelegate:self];
...
-(IBAction)brushButtonPressed:(UIButton *)sender {
    ...
    [self.canvasContainerView pause];
    //sender.enabled = NO;
    IMBrushPickerViewController *vc = [[IMBrushPickerViewController alloc] initWithNibName:@"IMBrushPickerViewController" bundle:[NSBundle mainBundle]];
    vc.delegate = self;
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        self.popover = [[UIPopoverController alloc] initWithContentViewController:vc];
        self.popover.backgroundColor = vc.view.backgroundColor;
...
-(IBAction)canvasButtonPressed:(UIButton *)sender {
    [self.canvasContainerView pause];
    IMCanvasPickerViewController *vc = [[IMCanvasPickerViewController alloc] initWithNibName:@"IMCanvasPickerViewController" bundle:[NSBundle mainBundle]];
  vc.delegate = self;
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        self.popover = [[UIPopoverController alloc] initWithContentViewController:vc];
        self.popover.backgroundColor = vc.view.backgroundColor;
...
-(IBAction)shareButtonPressed:(UIButton *)sender {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        NSLog(@"VC Bounds: w: %f, h: %f", vc.view.bounds.size.width, vc.view.bounds.size.height);
        vc.transitioningDelegate = self;
        vc.modalPresentationStyle = UIModalPresentationCustom;
        [self presentViewController:vc animated:YES completion:nil];
    }
...
-(IBAction)resetCanvasButtonPressed:(UIButton *)sender {
    [UIAlertView showOKCancelAlertWithMessage:NSLocalizedString(@"DO_YOU_WANT_TO_RESET", nil) OKBlock:^(void){
...
-(IBAction)settingsButtonPressed:(UIButton *)sender {
    [self.canvasContainerView pause];
    IMSettingsViewController *settingsViewController = [[IMSettingsViewController alloc] initWithNibName:@"IMSettingsViewController" bundle:nil];
...
- (void) snapDraggingPopoverToCorner {
    CGPoint upperLeft = CGPointMake(draggingPopover.bounds.size.width/2, draggingPopover.bounds.size.height/2);
    CGPoint upperRight = CGPointMake([draggingPopover superview].bounds.size.width - draggingPopover.bounds.size.width/2, draggingPopover.bounds.size.height/2);
    CGFloat distancetoupperLeft = ((draggingPopover.center.x - upperLeft.x)*(draggingPopover.center.x - upperLeft.x)) +
    ((draggingPopover.center.y - upperLeft.y)*(draggingPopover.center.y - upperLeft.y));
    CGFloat distancetoupperRight = ((draggingPopover.center.x - upperRight.x)*(draggingPopover.center.x - upperRight.x)) +
    ((draggingPopover.center.y - upperRight.y)*(draggingPopover.center.y - upperRight.y));
...
-(void) presentBrushEditor {
  ...
  vc.preferredContentSize = CGSizeMake(320, 480);
  vc.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(editPresetDoneButtonPressed:)];
  if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
    UIGestureRecognizer *panGestureRecoginizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePan:)];
    ...
    [self.presentedViewController presentViewController:nav2 animated:YES_PLEASE completion:nil];
  }
...
-(void)presentSequenceEditor {
  ...
  IMSequence *sequence = [IMSequence currentSequence];
    ...
    IMSequenceEditorTableViewController *seq = [[IMSequenceEditorTableViewController alloc] initWithSequence:sequence];
...
- (void)capturePaintingAndBlockAndPerformWithoutUpload:(void (^)(IMPainting *painting))action {
    [self dismissPopoverAndModalViewControllers];
    [UIView animateWithDuration:0.33f animations:^(void){ [self.heartButton setAlpha:0]; }];
    [self.canvasContainerView capturePaintingWithCompletionHandeler:^(IMPainting *painting) {
        action(painting);
        [UIView animateWithDuration:0.33f animations:^(void){ [self.heartButton setAlpha:1.0f]; }];
    }];
}

- (void)capturePaintingAndBlockAndPerform:(void (^)(IMPainting *painting))action {
    ...
    [self capturePaintingAndBlockAndPerformWithoutUpload:^(IMPainting *painting) {
        [self.canvasContainerView showProgressHUDWithString:NSLocalizedString(@"UPLOADING", nil)];
        [_networkManager uploadPainting:painting withCompletion:^(void) {
            [self.canvasContainerView hideProgressView];
            [UIView animateWithDuration:0.33f animations:^(void){ [self.heartButton setAlpha:1]; }];
            action(painting);
        } withFailure:^(void){
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"ERROR",nil) message:NSLocalizedString(@"UPLOAD_FAILED",nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"OK",nil) otherButtonTitles:nil];
            [alert show];
            [self.canvasContainerView hideProgressView];
            [UIView animateWithDuration:0.33f animations:^(void){ [self.heartButton setAlpha:1]; }];
            [UIView animateWithDuration:0.33f animations:^(void){ [self.progressLabel setAlpha:0]; }];
            [UIView animateWithDuration:0.33f animations:^(void){ [self.progressView setAlpha:0]; }];
        }];
    }];
#endif
}
...
- (void)emailButtonPressed:(id)sender {
    [self capturePaintingAndBlockAndPerform:^(IMPainting *painting) {
        ...
        if ([[vc class] canSendMail]) {
            ...
            NSString *link = [NSString stringWithFormat:@"<a href=\"%@\">%@</a>",painting.postURL,painting.postURL];
            NSString *body = [NSString stringWithFormat:NSLocalizedString(@"EMAIL_SHARING_HTML_{LINK}", nil),link];
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"ERROR",nil) message:NSLocalizedString(@"THIS_DEVICE_NOT_CONFIGURED_FOR_EMAIL",nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"OK",nil) otherButtonTitles:nil];
            [alert show];
        }
    }];
}
...
- (void)openInButtonPressed:(id)sender {
    [self capturePaintingAndBlockAndPerformWithoutUpload:^(IMPainting *painting) {
        ...
        if  (YES) {
            ...
            controller.completionHandler = ^void(NSString *activityType, BOOL completed) {
                [UIView animateWithDuration:0.33f animations:^(void){ [self.heartButton setAlpha:1]; }];
            };
            if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
                self.popover = [[UIPopoverController alloc] initWithContentViewController:controller];
                [self.popover presentPopoverFromRect:self.heartButton.frame inView:self.view permittedArrowDirections:UIPopoverArrowDirectionDown animated:YES];
            }
            else {
...
- (void)saveToPhotosButtonPressed:(id)sender {
    [self.canvasContainerView showProgressHUDWithString:NSLocalizedString(@"Saving...", nil)];
    [self capturePaintingAndBlockAndPerformWithoutUpload:^(IMPainting *painting) {
        UIImageWriteToSavedPhotosAlbum(painting.capturedImage, self, @selector(thisImage:hasBeenSavedInPhotoAlbumWithError:usingContextInfo:), NULL);
    }];
}

- (void)copyButtonPressed:(id)sender {
    [self.canvasContainerView showProgressHUDWithString:NSLocalizedString(@"Copying...", nil)];
    [self capturePaintingAndBlockAndPerformWithoutUpload:^(IMPainting *painting) {
        if (painting.postURL) {
        ...
        [self performSelector:@selector(copyDidFinish) withObject:nil afterDelay:1.0];
    }];
}
...
- (void)postCardButtonPressed:(id)sender {
    [self capturePaintingAndBlockAndPerformWithoutUpload:^(IMPainting *painting) {
        ...
        SYSincerelyController *controller = [[SYSincerelyController alloc] initWithImages:@[i]
...
- (void)facebookButtonPressed:(id)sender {
    [self capturePaintingAndBlockAndPerform:^(IMPainting *painting) {
        ...
        [composeViewController setInitialText:NSLocalizedString(@"FACEBOOK_SHARING_TEXT",nil)];
#if !DONT_UPLOAD_BEFORE_SHARING
        NSURL *url = [NSURL URLWithString:painting.postURL];
...
- (void)instagramButtonPressed:(UIImage *)sender {
    self.crossAppSharingController = [[IMCrossAppSharingController alloc] initWithServiceType:CrossAppSharingTypeInstagram];
    if (NO == [IMCrossAppSharingController isAppInstalledForServiceType:CrossAppSharingTypeInstagram]) {
        [self.crossAppSharingController promptUserToInstallAppUsingViewController:self completion:^(BOOL canceled) {
            [self dismissPopoverAndModalViewControllers];
        }];
    }
    else {
        [self capturePaintingAndBlockAndPerform:^(IMPainting *painting) {
            ...
            interactionController = [self.crossAppSharingController interactionControllerWithPainting:painting];
            interactionController.delegate = self;
            [interactionController presentOpenInMenuFromRect:self.heartButton.frame inView:self.view animated:YES];
        }];
    }
}

- (void)tumblrButtonPressed:(id)sender {
    self.crossAppSharingController = [[IMCrossAppSharingController alloc] initWithServiceType:CrossAppSharingTypeTumblr];
    
    if (NO == [IMCrossAppSharingController isAppInstalledForServiceType:CrossAppSharingTypeTumblr]) {
        [self.crossAppSharingController promptUserToInstallAppUsingViewController:self completion:^(BOOL canceled) {
            [self dismissPopoverAndModalViewControllers];
        }];
    }
...
- (void)twitterButtonPressed:(id)sender {
    [self capturePaintingAndBlockAndPerform:^(IMPainting *painting){
        ...
        [composeViewController setInitialText:NSLocalizedString(@"TWITTER_SHARING_TEXT",nil)];
#if !DONT_UPLOAD_BEFORE_SHARING
        NSURL *url = [NSURL URLWithString:painting.postURL];
        [composeViewController addURL:url];
#endif
...
- (void)thisImage:(UIImage *)image hasBeenSavedInPhotoAlbumWithError:(NSError *)error usingContextInfo:(void*)ctxInfo {
    ...
    if (error) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Error", nil)
                                                        message:NSLocalizedString(@"Error saving to photo library", nil)
                                                       delegate:nil
...
-(void)sharingViewControllerDidRequestRenderedImage:(IMSharingViewController *)sharingViewController {
    IMImageCropViewController *cropViewController = [[IMImageCropViewController alloc] initWithNibName:@"IMImageCropViewController" bundle:[NSBundle mainBundle]];
    cropViewController.delegate = sharingViewController;
    
    [self.canvasContainerView capturePaintingWithCompletionHandeler:^(IMPainting *painting) {
        cropViewController.cropImage = [painting capturedImage];
        [sharingViewController.navigationController pushViewController:cropViewController animated:YES_PLEASE];
    }];
}
...
- (void) imageSettingsDidChange:(NSNotification *) notification {
  [self.canvasContainerView createNewCanvasWithExistingImageUsingPresets:[[IMBrush currentBrush] json]];
}
